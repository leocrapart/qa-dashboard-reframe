...
+ avoids creating and maintaning an AzureDevops Test plan
+ gives more information than AzureDevops dashboards
...
* [`src/qa_dashboard_reframe/`](src/qa_dashboard_reframe/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/qa_dashboard_reframe/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `qa-dashboard-reframe` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/qa_dashboard_reframe/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns qa-dashboard-reframe.example
  (:require [qa-dashboard-reframe.config :as config])
